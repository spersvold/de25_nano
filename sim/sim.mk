...
VCS_FLAGS	:= -q -nc -debug_acc+pp -timescale=1ns/1ps -sverilog -assert svaext +define+HAVE_VCDPLUSON +verilog2001ext+.v
...
compile:	$(PRELUDE) Makefile sim_files.f
		...
		$(AT)msgcfg=$$(mktemp $${TMPDIR:-/tmp}/msg_config.XXXXXX); \
		   trap 'rm -f "$$msgcfg"' EXIT; \
		   ipfiles=$$(grep -vE '^"?[-+]' unique_ip_files.f | sed -e 's/^"//' -e 's/"$$//' -e 's://*:/:g' -e 's/^/ +file=/' -e 's/$$/;/'); \
		   awk -v ins="$$ipfiles" '/^[ \t]*\/\/@IP_FILES@[ \t]*$$/ { print ins; next } { print }' $(MSG_CONFIG) > "$$msgcfg"; \
		   $(VCS) $(VCS_FLAGS) -msg_config="$$msgcfg" $(BUILDARGS) -F sim_files.f -o $(SIMBIN) -l compile.log
